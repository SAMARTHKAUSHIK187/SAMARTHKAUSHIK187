<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Samarth%20Kaushik&fontSize=55&fontColor=fff&animation=twinkling&fontAlignY=35&desc=B.Tech%20CSE%20%7C%20IIITDM%20Kurnool&descAlignY=55&descSize=20" width="100%"/>

</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=22&pause=800&color=58A6FF&center=true&vCenter=true&width=750&height=50&lines=⚡+Competitive+Programmer;🧠+DSA+%26+Algorithm+Enthusiast;🔧+Digital+Logic+%26+Systems+Builder;📈+Quantitative+Finance+Explorer;🤖+IoT+%26+Embedded+Systems+Tinkerer)](https://git.io/typing-svg)

</div>

<div align="center">

<img src="https://komarev.com/ghpvc/?username=Samarth-Kaushik&label=Profile+Views&color=blueviolet&style=for-the-badge" alt="Profile Views" />
&nbsp;
<img src="https://img.shields.io/github/followers/Samarth-Kaushik?label=Followers&style=for-the-badge&color=blue" alt="Followers" />
&nbsp;
<img src="https://img.shields.io/badge/Open%20To-Opportunities-brightgreen?style=for-the-badge" alt="Open to Work" />

</div>

<br>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

---

## 🚀 About Me

<img align="right" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="260" alt="Coding GIF"/>

```yaml
Name     : Samarth Kaushik
College  : IIITDM Kurnool
Degree   : B.Tech — Computer Science & Engineering
Focus    : Data Structures & Algorithms
Weapons  : C++ ⚔️  Python 🐍  Verilog 🔌
Interests:
  - Competitive Programming 🏆
  - Quantitative Finance 📈
  - Digital Logic & Systems 💡
  - Military & Defense Tech 🛡️
  - IoT & Embedded Systems 🤖
Off-Screen:
  - Volleyball Court 🏐
  - Running Tracks 🏃
  - Debate Stages 🎙️
```

<br clear="right"/>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

---

## 📁 Featured Projects

<details>
<summary><b>🗜️ Huffman File Compressor &nbsp; &nbsp; <img src="https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white"/> &nbsp; <img src="https://img.shields.io/badge/Status-Complete-brightgreen?style=flat-square"/></b></summary>

<br>

> Lossless file compression & decompression using the Huffman Coding algorithm — built entirely in C++

```
📦 Huffman-File-Compressor
 ├── 🌳 Huffman Tree Builder   → Greedy prefix tree with character frequency analysis
 ├── 📊 Data Structures        → Min-Priority Queue (Heap) + Unordered Hash Maps
 ├── ⚡ Complexity             → O(n log n) build time | O(1) code retrieval
 ├── 💾 Binary Streams         → Fast read/write for any file type
 └── 🔁 Serialization          → Tree embedded into compressed file for reconstruction
```

| Feature | Detail |
|:---|:---|
| 🧠 **Algorithm** | Greedy Huffman Tree — high-freq chars get shorter codes |
| 🏗️ **Core DS** | Min-Heap for tree construction, HashMap for O(1) lookups |
| 🚀 **Performance** | Minimized storage footprint with optimized bitwise encoding |
| 🔄 **Decompression** | Flawless data reconstruction via serialized tree |

</details>

<hr style="height:1px;border:none;background-color:#30363d;margin:10px 0;">

<details>
<summary><b>🥚 Project E.G.G. — Expression to Gate Generator &nbsp; &nbsp; <img src="https://img.shields.io/badge/Digital%20Logic-158223?style=flat-square"/> &nbsp; <img src="https://img.shields.io/badge/Status-Complete-brightgreen?style=flat-square"/></b></summary>

<br>

> Bridges the gap between Boolean algebra and hardware — converts expressions directly to logic gate schemas

```
🔌 Project E.G.G.
 ├── 🔤 Expression Parser      → Handles AND, OR, NOT, XOR from raw boolean input
 ├── 📐 Gate Synthesizer       → Auto-generates equivalent hardware logic gate schema
 ├── 📋 Truth Table Engine     → Instant multi-variable truth table generation
 ├── 🔄 Infix → Postfix        → Stack-based conversion for clean syntax trees
 └── ✅ Verilog Verifier       → Aids in circuit reduction & combinatorial evaluation
```

| Feature | Detail |
|:---|:---|
| 🔍 **Parsing** | Raw boolean expressions → hardware gate schema |
| 📊 **Truth Tables** | Instant generation for any multi-variable expression |
| 🔧 **Use Case** | Verilog verification, circuit reduction, state evaluation |
| 🧮 **Implementation** | Stack-based infix-to-postfix + syntax tree evaluation |

</details>

<hr style="height:1px;border:none;background-color:#30363d;margin:10px 0;">

<details>
<summary><b>🤖 Fire-Bot — Autonomous IoT Fire Extinguisher &nbsp; &nbsp; <img src="https://img.shields.io/badge/IoT-FF9800?style=flat-square"/> &nbsp; <img src="https://img.shields.io/badge/Status-Complete-brightgreen?style=flat-square"/></b></summary>

<br>

> Autonomous hardware robot that detects, navigates to, and extinguishes fires — with real-time IoT alerts

```
🤖 Fire-Bot
 ├── 🔥 Flame Sensors          → Multi-directional indoor fire outbreak detection
 ├── ⚙️  Motor Drivers          → Autonomous navigation toward hazard source
 ├── 💧 Actuators              → Real-time extinguishing mechanism deployment
 ├── 📡 Wi-Fi Module           → Instant alert notifications to homeowners
 └── 🧠 Embedded Logic         → Low-level hardware programming for automated response
```

| Feature | Detail |
|:---|:---|
| 🌡️ **Sensing** | Multi-directional flame sensors + microcontrollers |
| 🚗 **Navigation** | Motor drivers autonomously guide bot to fire source |
| 📲 **IoT Alerts** | Wi-Fi module pushes instant notifications on detection |
| 🔩 **Integration** | Embedded systems + circuit modeling + hardware programming |

</details>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

---

## 🏆 Competitive Programming

<div align="center">

<img src="https://img.shields.io/badge/Codeforces-Samarth__Kaushik-1F8ACB?style=for-the-badge&logo=codeforces&logoColor=white" />
&nbsp;
<img src="https://img.shields.io/badge/LeetCode-kaushik__samarth-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" />
&nbsp;
<img src="https://img.shields.io/badge/CodeChef-coder__kaushik-5B4638?style=for-the-badge&logo=codechef&logoColor=white" />

</div>

<br>

<div align="center">

| 🌐 Platform | 🔗 Profile | 🎯 Focus |
|:---:|:---:|:---:|
| **Codeforces** | [Samarth_Kaushik](https://codeforces.com/profile/Samarth_Kaushik) | Rated Contests & Div. Problems |
| **LeetCode** | [kaushik_samarth](https://leetcode.com/u/kaushik_samarth) | DSA Problem Solving |
| **CodeChef** | [coder_kaushik](https://www.codechef.com/users/coder_kaushik) | Long & Short Contests |
| **Codolio** | [samarth_kaushik](https://codolio.com/profile/samarth_kaushik) | Combined Stats Dashboard |

</div>

<br>

```
🔑 Key Focus Areas
┌──────────────────────────────────────────────────────┐
│  🔍 Binary Search      │  🔢 Bitwise Operations       │
│  🗂️  Sorting Algos     │  📐 Mathematical Modelling   │
│  ⏱️  Time Complexity   │  🌲 Trees & Graphs           │
└──────────────────────────────────────────────────────┘
```

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

---

## 🛠️ Technical Arsenal

<div align="center">

**Languages**

![C++](https://img.shields.io/badge/C++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Python](https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![Verilog](https://img.shields.io/badge/Verilog-%23158223.svg?style=for-the-badge&logoColor=white)

<br>

**Core Skills**

![Data Structures](https://img.shields.io/badge/Data%20Structures-8A2BE2?style=for-the-badge)
![Algorithms](https://img.shields.io/badge/Algorithms-FF5722?style=for-the-badge)
![Complexity Analysis](https://img.shields.io/badge/Complexity%20Analysis-00BCD4?style=for-the-badge)
![Bitwise Ops](https://img.shields.io/badge/Bitwise%20Operations-4CAF50?style=for-the-badge)
![Binary Search](https://img.shields.io/badge/Binary%20Search-FF9800?style=for-the-badge)
![Graph Theory](https://img.shields.io/badge/Graph%20Theory-9C27B0?style=for-the-badge)

<br>

**Domains**

![Competitive Programming](https://img.shields.io/badge/Competitive%20Programming-E91E63?style=for-the-badge&logo=leetcode&logoColor=white)
![Digital Systems](https://img.shields.io/badge/Digital%20Systems-607D8B?style=for-the-badge)
![IoT Integration](https://img.shields.io/badge/IoT%20Integration-FF9800?style=for-the-badge)
![Quantitative Finance](https://img.shields.io/badge/Quantitative%20Finance-009688?style=for-the-badge)
![Embedded Systems](https://img.shields.io/badge/Embedded%20Systems-795548?style=for-the-badge)

<br>

**Tools & Environments**

![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

</div>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

---

## 📊 GitHub Stats & Activity

<div align="center">

<p>
  <img src="https://github-readme-stats.vercel.app/api?username=Samarth-Kaushik&show_icons=true&hide_border=false&border_radius=15&theme=tokyonight&border_color=58A6FF&bg_color=0D1117&title_color=58A6FF&icon_color=F8D866" alt="GitHub Stats" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Samarth-Kaushik&layout=compact&hide_border=false&border_radius=15&theme=tokyonight&border_color=58A6FF&bg_color=0D1117&title_color=58A6FF" alt="Top Languages" width="48%" />
</p>

<p>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Samarth-Kaushik&theme=tokyonight&hide_border=false&border_radius=15&border=58A6FF&background=0D1117&ring=58A6FF&fire=F8D866&currStreakLabel=58A6FF" alt="GitHub Streak" width="60%" />
</p>

</div>

---

## 📈 Contribution Graph

<div align="center">

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Samarth-Kaushik&bg_color=0D1117&color=58A6FF&line=58A6FF&point=F8D866&area=true&hide_border=false&border_radius=15&theme=react-dark)](https://github.com/Samarth-Kaushik)

</div>

---

## 🏅 GitHub Trophies

<div align="center">

[![Trophies](https://github-profile-trophy.vercel.app/?username=Samarth-Kaushik&theme=tokyonight&no-frame=false&no-bg=false&margin-w=6&column=7)](https://github.com/ryo-ma/github-profile-trophy)

</div>

---

## 🐍 Contribution Snake

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Samarth-Kaushik/Samarth-Kaushik/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Samarth-Kaushik/Samarth-Kaushik/output/github-contribution-grid-snake.svg">
    <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/Samarth-Kaushik/Samarth-Kaushik/output/github-contribution-grid-snake.svg" width="100%">
  </picture>
</div>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%">

---

## 📫 Connect with Me

<div align="center">

<a href="https://www.linkedin.com/in/samarth-kaushik-12b946381">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
&nbsp;
<a href="mailto:samarth_kaushik@zohomail.in">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>
&nbsp;
<a href="https://codeforces.com/profile/Samarth_Kaushik">
  <img src="https://img.shields.io/badge/Codeforces-1F8ACB?style=for-the-badge&logo=codeforces&logoColor=white" alt="Codeforces" />
</a>
&nbsp;
<a href="https://leetcode.com/u/kaushik_samarth">
  <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode" />
</a>
&nbsp;
<a href="https://www.codechef.com/users/coder_kaushik">
  <img src="https://img.shields.io/badge/CodeChef-5B4638?style=for-the-badge&logo=codechef&logoColor=white" alt="CodeChef" />
</a>
&nbsp;
<a href="https://codolio.com/profile/samarth_kaushik">
  <img src="https://img.shields.io/badge/Codolio-6C63FF?style=for-the-badge&logoColor=white" alt="Codolio" />
</a>

</div>

<br>

<div align="center">

> *"The goal isn't to live forever, the goal is to create something that does."*

</div>

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer" width="100%"/>
